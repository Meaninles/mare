...
func (service *Service) RestoreAsset(ctx context.Context, request RestoreAssetRequest) (RestoreAssetSummary, error) {
	task, err := service.createCatalogTask(ctx, taskTypeRestoreAsset, request)
	if err != nil {
	...
	if restoreErr != nil {
		...
		slog.Error("asset restore failed", "taskId", task.ID, "assetId", request.AssetID, "error", restoreErr)
		return summary, restoreErr
	}

	summary.AssetID = result.asset.ID
	summary.DisplayName = result.asset.DisplayName
	summary.SourceEndpointID = result.sourceEndpoint.ID
	summary.SourceEndpointName = result.sourceEndpoint.Name
	...
	summary.CreatedReplica = result.createdReplica
	...
	slog.Info(
		...
		"sourceEndpointId", summary.SourceEndpointID,
...
func (service *Service) RestoreAssetsToEndpoint(ctx context.Context, request BatchRestoreRequest) (BatchRestoreSummary, error) {
	task, err := service.createCatalogTask(ctx, taskTypeRestoreBatch, request)
	if err != nil {
	...
	targetEndpoint, err := service.store.GetStorageEndpointByID(ctx, request.TargetEndpointID)
	if err != nil {
		return service.failBatchRestoreTask(ctx, task, summary, startedAt, err)
	}
	summary.TargetEndpointName = targetEndpoint.Name
	...
	for _, assetID := range assetIDs {
		...
		if sourceErr != nil {
			itemResult.Error = sourceErr.Error()
			summary.FailedCount++
			summary.Items = append(summary.Items, itemResult)
			continue
		}

		itemResult.DisplayName = asset.DisplayName
		itemResult.SourceEndpointID = sourceEndpoint.ID

		result, restoreErr := service.executeRestoreAsset(ctx, RestoreAssetRequest{
			AssetID:          assetID,
		...
		itemResult.DisplayName = result.asset.DisplayName
		itemResult.SourceEndpointID = result.sourceEndpoint.ID
		itemResult.Status = taskStatusSuccess
		itemResult.Skipped = result.skipped
		if result.skipped {
	...
	case summary.FailedCount > 0:
		summary.Status = taskStatusFailed
	case summary.SuccessCount > 0 || summary.SkippedCount > 0:
		summary.Status = taskStatusSuccess
	default:
		summary.Status = taskStatusFailed
	}
	...
	if summary.FailedCount > 0 {
		message := fmt.Sprintf("%d assets failed during restore batch", summary.FailedCount)
	...
	if summary.FailedCount > 0 {
		slog.Warn(
			"batch restore completed with failures",
			"taskId", task.ID,
			"targetEndpointId", summary.TargetEndpointID,
		...
		return summary, errors.New("batch restore completed with failures")
	}
