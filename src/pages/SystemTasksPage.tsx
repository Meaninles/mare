...
import type { CatalogTransferTaskRecord } from "../types/catalog";
...
export function SystemTasksPage() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    return [...tasks]
      ...
      .sort(compareTransferTasksByTime);
  }, [directionFilter, searchValue, statusFilter, tasks]);

  const tasksPagination = usePagination(filteredTasks, 20);
  const pagedTasks = tasksPagination.pagedItems;
  const visibleTaskIds = useMemo(() => pagedTasks.map((task) => task.id), [pagedTasks]);
  ...
  async function handlePrioritize(taskIds: string[]) {
    const candidates = taskIds
      ...
      .filter((task): task is CatalogTransferTaskRecord => Boolean(task) && canPrioritizeDownload(task as CatalogTransferTaskRecord));
    ...
    <section className="page-stack system-tasks-page">
      ...
      {!isLibraryOpen ? (
        <article className="detail-card empty-state">
          ...
          <div>
            <h4>请先打开一个资产库</h4>
            <p>任务页现在只显示当前资产库中的任务列表，需要先进入资产库后才能查看。</p>
          </div>
        </article>
      ) : (
              ...
              <table className="transfer-task-table">
                ...
                <tbody>
                  {pagedTasks.map((task) => {
                    ...
                    return (
                      <tr key={task.id} className={`transfer-task-row${isSelected ? " is-selected" : ""}`}>
                        <td className="transfer-selection-cell">
                          <input
                            type="checkbox"
                            checked={isSelected}
...
function compareTransferTasksByTime(left: CatalogTransferTaskRecord, right: CatalogTransferTaskRecord) {
  const rightTime = resolveTransferTaskTime(right);
  const leftTime = resolveTransferTaskTime(left);
...
function resolveTransferTaskTime(task: CatalogTransferTaskRecord) {
  for (const value of [task.updatedAt, task.startedAt, task.createdAt]) {
